# Credit: https://github.com/lucidrains/denoising-diffusion-pytorch/blob/main/denoising_diffusion_pytorch/denoising_diffusion_pytorch_1d.py
import math
...
import torch
from torch import __version__, nn, einsum, Tensor
...
class Unet1D(Module):
    def __init__(
        ...
        self.init_conv = nn.Conv1d(input_channels, init_dim, 7, padding = 3) # type: ignore
        ...
        self.final_res_block = resnet_block(init_dim * 2, init_dim) # type: ignore
        self.final_conv = nn.Conv1d(init_dim, self.out_dim, 1) # type: ignore

    def forward(self, x, time, x_self_cond = None):
        if self.self_condition:
            x_self_cond = default(x_self_cond, lambda: torch.zeros_like(x))
            x = torch.cat((x_self_cond, x), dim = 1) # type: ignore # type: ignore
...
class GaussianDiffusion1D(Module):
    def __init__(
        ...
        *,
        ...
        self.is_ddim_sampling = self.sampling_timesteps < timesteps # type: ignore
